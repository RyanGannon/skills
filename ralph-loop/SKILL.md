...
- **`ralph/prompt.md`** — Instructions for the agent each iteration. Crucially includes the feedback loop commands (tests, type checker, linter) and the rule `ONLY WORK ON A SINGLE TASK`. Also includes the `<promise>NO MORE TASKS</promise>` termination instruction, a RESUMPTION check (see below), and a STAY UNATTENDED section telling the agent it must never stop to ask a question — a task's own acceptance criteria are the human's advance authorization for whatever they explicitly call for.
...
**Resumability has a gap the RESUMPTION section closes:** the loop's continuity model assumes all state lives in the injected commits (only the last 5), but an iteration interrupted mid-task (a `claude` crash, a usage-limit hit) can leave *uncommitted* work, or a commit made but never pushed/PR'd, that isn't in that injected context. Without an explicit check, a fresh iteration has no way to discover it and may start a second task on top of the WIP, or ignore it entirely. The RESUMPTION section in the `prompt.md` template (checking `git status`/branch before task selection, for both cases) closes this.

**STAY UNATTENDED closes a related gap:** the loop has no one watching a `--print` run in real time, so an agent that stops mid-iteration to ask a question (e.g. on a sandbox/permission rejection) just hangs until a human happens to read the transcript later — which can be hours. The STAY UNATTENDED section in the `prompt.md` template tells the agent a task's own acceptance criteria are the human's advance authorization for whatever they explicitly call for, so it should proceed rather than stop when the gated action is exactly what the task asked for.
...
- `afk.sh` must check `claude`'s actual exit code (`${PIPESTATUS[0]}`) and stop with a diagnostic on failure — a swallowed crash/usage-limit hit otherwise burns the rest of the iteration budget doing nothing
- Resumability via injected commits has a gap: uncommitted WIP, or a commit made but never pushed/PR'd, from an interrupted iteration isn't in the last-5-commits context, so `ralph/prompt.md` needs an explicit RESUMPTION check for both before picking a new task
...
- Ralph must never stop an iteration to ask a question or wait on a permission prompt — it runs unattended via `claude --print` with no one there to answer, so an asked question just hangs until a human happens to read the transcript later. A task's own acceptance criteria are the human's advance authorization for whatever they explicitly call for, including actions that would otherwise need a sandbox/permission exception (e.g. a task whose acceptance criteria require a real run against a live external service) — the agent should proceed using that authorization rather than stopping to ask (see the STAY UNATTENDED section in the prompt.md template)
